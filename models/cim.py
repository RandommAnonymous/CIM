...
class CIM(nn.Module):
    def __init__(self, config=None, load_vision_params=False, load_text_params=False,
        ...
        if config['gpt']:
            gpt_num_head = 8
            gpt_num_layer = 2
            self.img_caption = coAttenTransformer(config['embed_dim'], gpt_num_head, gpt_num_layer)
            self.gpt_caption = coAttenTransformer(config['embed_dim'], gpt_num_head, gpt_num_layer)
